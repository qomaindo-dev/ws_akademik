...
class Siswa extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model('Siswa_db','Siswa');
	...
	public function index()
	{
		...
		$data['content'] = $this->load->view('siswa/siswa',$data,true);
  ...
  public function ajax_list() {
			...
			foreach ($list as $siswa) {
				...
				$row[] = $siswa->kelas;
				$row[] = $siswa->no_tlp;
				$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit('."'".$siswa->nis."'".')"><i class="fa fa-pencil"></i> Edit</a>
					  		  <a class="btn btn-sm btn-danger" href="javascript:void(0)" title="Delete" onclick="deleted('."'".$siswa->nis."'".')"><i class="fa fa-trash"></i> Hapus</a>
					  		  <a class="btn btn-sm btn-default" href="'.base_url().'page_admin/siswa/siswaDetail/'.$siswa->nis.'" title="Detail"><i class="fa fa-search"></i> Detail</a>';
				$data[] = $row;
			}
			$output = array(
							"draw" => $_POST['draw'],
      ...
      if($this->input->post('nisn') == '') {
          ...
          $data['status'] = FALSE;
			...
			if($this->input->post('tgl_lahir') == '') {
				...
				$data['error_string'][] = 'Tanggal Lahir  kosong';
				$data['status'] = FALSE;
			}
